...
// W_PRIX = 0.20 par défaut, remplacé par W_PRIX_PREMIUM en mode budget illimité
...
function couche1(raquette, quiz) {
  ...
  const budgetIllimite = budget >= BUDGET_ILLIMITE

  if (Object.keys(schema).length === 0) return { ok: false, raison: 'pas_de_schema' }
  if (raquette.stock <= 0) return { ok: false, raison: 'stock_zero' }
  // En mode budget illimité on ne filtre pas par prix
  if (!budgetIllimite && raquette.price > budget) return { ok: false, raison: 'hors_budget' }
...
function couche2(pool, budget) {
  ...
  return pool.map(r => {
    ...
    let scorePrix
    if (budgetIllimite) {
      // Budget illimité : favorise les raquettes plus chères (premium)
      scorePrix = normalise(r.price, minP, maxP)
    } else {
      // Budget défini : favorise les raquettes proches du budget max
      scorePrix = Math.min(100, (r.price / budget) * 100)
    }
...
export function scoreRaquettes(raquettes, quiz, topN = 12) {
  const sensations = Array.isArray(quiz.sensation) ? quiz.sensation : [quiz.sensation]
