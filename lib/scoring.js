...
// --- Score expliqué : 2 points forts (+) et 1 point faible (−) par raquette ---
const PHRASE_PLUS = {
  Puissance: 'Puissance de balle',
  Confort: 'Confort au bras',
  Spin: 'Effet / lift facile',
  'Contrôle': 'Contrôle et précision',
  'Tolérance': 'Tolérance au décentrage',
  'Maniabilité': 'Maniabilité',
}
const PHRASE_MOINS = {
  Puissance: 'Moins de puissance brute',
  Confort: 'Moins tolérante pour le bras',
  Spin: "Moins d'effet",
  'Contrôle': 'Moins de précision',
  'Tolérance': 'Zone de frappe plus étroite',
  'Maniabilité': 'Moins maniable',
}

function expliquerRaquette(schema, poids) {
  const s = schema || {}
  const tries = DIMS.map(d => ({ d, v: s[d] || 0 })).sort((a, b) => b.v - a.v)
  const plus = tries.slice(0, 2).map(x => PHRASE_PLUS[x.d])
  // Point faible : signaler le poids si la raquette est lourde, sinon la dimension la plus basse.
  const pc = poidsComparable(poids)
  let moins
  if (pc != null && pc >= 368) {
    moins = (typeof poids === 'string' && /\d/.test(poids)) ? `Un peu lourde : ${poids}` : 'Un peu lourde'
  } else {
    moins = PHRASE_MOINS[tries[tries.length - 1].d]
  }
  return { plus, moins }
}
...
export function scoreRaquettes(raquettes, quiz, topN = 10, gene = 'aucune') {
  ...
  const final = top.slice(0, topN)
  final.forEach(r => { r.explication = expliquerRaquette(r.schema, r.poids) })
  return final
}
