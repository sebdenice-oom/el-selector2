...
// Convertit un champ `poids` (grammes "355-360", "365 g" ou libellé "léger/moyen/lourd") en valeur comparable.
function poidsComparable(p) {
  if (p == null || p === '') return null
  if (typeof p === 'number') return p
  const s = String(p).toLowerCase()
  const nums = s.match(/\d+([.,]\d+)?/g)
  if (nums && nums.length) return Math.max(...nums.map(n => parseFloat(n.replace(',', '.'))))
