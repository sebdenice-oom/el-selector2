...
export function scoreRaquettesUpgrade(raquettes, cibles, ciblesInitiales, prixReference, raquetteExclueId = null, topN = 12) {
  ...
  const scoredJoffrey = pool.map(r => {
    ...
    return { ...r, _scoreJF: scoreJF, _scoreMarge: scoreMarge }
  ...
  const SEUIL_MARGE = 50
  const ECART_MAX   = 10

  // Candidat Team EPS : meilleure marge, différent du top1, conditions strictes
  const epsCandidat = scoredJoffrey.find(r => {
    ...
    if (top1Score - scoreBase.scoreFinal > ECART_MAX) return false
    if ((r._scoreMarge || 0) < SEUIL_MARGE) return false
    if (r.stock <= 0 && !r.precommande) return false
    return true
  })

  const top = scored.filter(r => r.id !== epsCandidat?.id).slice(0, topN)

  if (epsCandidat) {
    const base = scored.find(r => r.id === epsCandidat.id) || epsCandidat
    top.splice(1, 0, { ...base, isEPS: true })
  }
