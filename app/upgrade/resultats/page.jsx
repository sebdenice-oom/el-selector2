      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 3 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{
            ...
            color: atteint ? '#1D9E75' : '#D85A30',
          }}>
            {valeurRaquette}
            <span style={{ fontSize: 9, marginLeft: 2 }}>{atteint ? '✓' : '↑'}</span>
          </span>
        </div>
      </div>
      <div style={{ position: 'relative', height: 6, background: '#EEF0F6', borderRadius: 3, overflow: 'visible' }}>
        {/* Barre valeur actuelle (grise) */}
        <div style={{
          position: 'absolute', left: 0, top: 0, bottom: 0,
          width: `${valeurActuelle}%`, background: '#D8DCF0',
          borderRadius: 3,
        }} />
        {/* Barre raquette candidate */}
        <div style={{
          position: 'absolute', left: 0, top: 0, bottom: 0,
          width: `${valeurRaquette}%`, background: couleur,
        ...
        <div style={{
          position: 'absolute',
          left: `calc(${valeurCible}% - 1px)`,
          top: -2, bottom: -2,
          width: 2.5,
          background: atteint ? '#1D9E75' : '#D85A30',
          borderRadius: 2,
          zIndex: 2,
        }} />
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 3 }}>
        <span style={{ fontFamily: 'Nunito, sans-serif', fontSize: 9, color: '#ccc', fontWeight: 600 }}>0</span>
        <span style={{ fontFamily: 'Nunito, sans-serif', fontSize: 9, color: '#ccc', fontWeight: 600 }}>100</span>
      </div>
    </div>
  )
}
...
function RaquetteCard({ raquette, rank, raquetteActuelle, cibles }) {
  ...
  const dimsDisponibles = DIMS.filter(d => raquette.schema[d] !== undefined)
            ...
            <div style={{ padding: '14px 16px 16px', borderTop: '1px solid #EEF0F6', background: '#FAFBFF' }}>
              ...
              <div style={{ display: 'flex', gap: 16, marginBottom: 12, flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  <div style={{ width: 12, height: 5, background: '#D8DCF0', borderRadius: 2 }} />
                  <span style={{ fontFamily: 'Nunito, sans-serif', fontSize: 10, fontWeight: 600, color: '#aaa' }}>Ta raquette actuelle</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  <div style={{ width: 12, height: 5, background: '#2B4EE5', borderRadius: 2, opacity: 0.85 }} />
                  <span style={{ fontFamily: 'Nunito, sans-serif', fontSize: 10, fontWeight: 600, color: '#aaa' }}>Cette raquette</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  <div style={{ width: 3, height: 12, background: '#D85A30', borderRadius: 1 }} />
                  <span style={{ fontFamily: 'Nunito, sans-serif', fontSize: 10, fontWeight: 600, color: '#aaa' }}>Ton objectif</span>
                </div>
              </div>
              {dimsDisponibles.map(d => (
                <ComparaisonBar
                  key={d}
                  dim={d}
                  valeurActuelle={raquetteActuelle?.schema?.[d] ?? 50}
                  valeurCible={cibles?.[d] ?? 50}
                  valeurRaquette={raquette.schema[d]}
                />
              ))}
            </div>
          )}
...
// --- Panneau sliders latéral ---
function PanneauSliders({ cibles, raquetteActuelle, onChangeCible, ouvert, onToggle }) {
  return (
    <>
      {/* Bouton flottant */}
      <button
        ...
        style={{
          ...
          background: '#2B4EE5', color: '#fff',
          border: 'none', borderRadius: '12px 0 0 12px',
          padding: '14px 10px', cursor: 'pointer', zIndex: 200,
          fontFamily: 'Nunito, sans-serif', fontSize: 11, fontWeight: 800,
          display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
          boxShadow: '-2px 0 12px rgba(43,78,229,0.2)',
          transition: 'background .15s',
        }}>
        <span style={{ fontSize: 16 }}>🎚️</span>
        <span style={{ writingMode: 'vertical-rl', textOrientation: 'mixed', transform: 'rotate(180deg)', letterSpacing: '0.05em' }}>
          {ouvert ? 'Fermer' : 'Ajuster'}
        </span>
      </button>

      {/* Panneau */}
      <div style={{
        position: 'fixed', right: ouvert ? 0 : '-320px', top: 0, bottom: 0,
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <span style={{ fontFamily: 'Nunito, sans-serif', fontSize: 15, fontWeight: 900, color: '#1A1A2E' }}>
          ...
          return (
            <div key={d} style={{ marginBottom: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ fontFamily: 'Nunito, sans-serif', fontSize: 13, fontWeight: 800, color: '#1A1A2E', display: 'flex', alignItems: 'center', gap: 5 }}>
                  {DIM_ICONS[d]} {d}
                </span>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ fontFamily: 'Nunito, sans-serif', fontSize: 16, fontWeight: 900, color: couleur, minWidth: 28, textAlign: 'right' }}>
                    {valeurCible}
                  </span>
                </div>
              </div>
        ...
        <button
          ...
          style={{
            ...
            color: '#888', cursor: 'pointer', marginTop: 8,
          }}>
...
export default function UpgradeResultatsPage() {
  ...
  // Charger le catalogue et les données de session
  useEffect(() => {
    const ra = sessionStorage.getItem('upgrade_raquette')
    const c = sessionStorage.getItem('upgrade_cibles')
    const cat = sessionStorage.getItem('upgrade_catalogue')

    const raquette = ra ? JSON.parse(ra) : null
    const ciblesInit = c ? JSON.parse(c) : {}
    setRaquetteActuelle(raquette)
    setCibles(ciblesInit)

    if (cat) {
      const catalogueParse = JSON.parse(cat)
      setCatalogue(catalogueParse)
      const top = scoreRaquettesUpgrade(catalogueParse, ciblesInit, raquette?.id)
      setResultats(top)
      setLoading(false)
    } else {
      ...
      fetch('/api/catalogue')
        .then(r => r.json())
        .then(data => {
          const cat = data.raquettes || []
          setCatalogue(cat)
          sessionStorage.setItem('upgrade_catalogue', JSON.stringify(cat))
          const top = scoreRaquettesUpgrade(cat, ciblesInit, raquette?.id)
          setResultats(top)
          setLoading(false)
        })
  ...
  // Recalcul automatique quand les cibles changent
  const handleChangeCible = useCallback((dim, valeur) => {
    setCibles(prev => {
      const newCibles = { ...prev, [dim]: valeur }
      // Recalcul immédiat côté client
      const top = scoreRaquettesUpgrade(catalogue, newCibles, raquetteActuelle?.id)
      setResultats(top)
      setVisibles(PAGE_SIZE) // reset pagination
      // Sauvegarder en session
      sessionStorage.setItem('upgrade_cibles', JSON.stringify(newCibles))
      return newCibles
    })
  }, [catalogue, raquetteActuelle])
    ...
    return (
      <main style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#F8F9FB' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontFamily: 'Nunito, sans-serif', fontWeight: 700, color: '#888' }}>Calcul des meilleures évolutions…</p>
        </div>
      </main>
    )
  }
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#F8F9FB', paddingBottom: 80 }}>
      ...
      <div style={{ background: '#EEF2FF', padding: '24px 20px 20px', borderBottom: '1px solid #D8E0FA' }}>
        <div style={{ maxWidth: 680, margin: '0 auto' }}>
          <h1 style={{ fontFamily: 'Nunito, sans-serif', fontSize: 'clamp(20px, 5vw, 28px)', fontWeight: 900, color: '#1A1A2E', marginBottom: 8 }}>
            Tes raquettes d'évolution 🚀
          </h1>
          {raquetteActuelle && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <span style={{ fontFamily: 'Nunito, sans-serif', fontSize: 12, fontWeight: 700, color: '#2B4EE5', opacity: 0.7 }}>
                Depuis :
              </span>
              <span style={{ fontFamily: 'Nunito, sans-serif', fontSize: 12, fontWeight: 800, color: '#2B4EE5', background: 'rgba(43,78,229,0.08)', padding: '2px 10px', borderRadius: 100 }}>
                {raquetteActuelle.title}
              </span>
              <button
                onClick={() => router.push('/upgrade')}
                style={{ fontFamily: 'Nunito, sans-serif', fontSize: 11, fontWeight: 700, color: '#888', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>
                Changer
              </button>
            </div>
          )}
          <p style={{ fontFamily: 'Nunito, sans-serif', fontSize: 12, color: '#2B4EE5', fontWeight: 600, marginTop: 8, opacity: 0.7 }}>
            🎚️ Utilise le panneau latéral pour ajuster tes critères en temps réel
          </p>
        </div>
      </div>
      ...
      <div className="container" style={{ paddingTop: 24, paddingRight: 52 }}>
        {resultats.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 0' }}>
            ...
            <p style={{ color: '#888899', marginBottom: 24, fontFamily: 'Nunito, sans-serif', fontWeight: 600, fontSize: 14 }}>
              Essaie d'ajuster tes objectifs avec le panneau latéral.
            </p>
            <button
          ...
          <>
            ...
            {raquettesVisibles.map((r, i) => (
              <RaquetteCard
                ...
                raquette={r}
                rank={i + 1}
                raquetteActuelle={raquetteActuelle}
                cibles={cibles}
              />
            ))}
            ...
            {peutVoirPlus && (
              <button
                ...
                style={{
                  ...
                  color: '#2B4EE5', cursor: 'pointer', marginBottom: 20,
                }}>
            ...
            <div style={{ textAlign: 'center', marginTop: 16, paddingTop: 16, borderTop: '1px solid #E8EAF0' }}>
              <button onClick={() => router.push('/upgrade')} className="btn btn-secondary" style={{ display: 'inline-flex', width: 'auto' }}>
